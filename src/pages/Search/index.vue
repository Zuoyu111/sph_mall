                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span v-show="isOne">
                      &nbsp;
                      <span v-show="isAsc">上</span>
                      <span v-show="isDesc">下</span>
                    </span></a
                  >
                </li>
...
export default {
  ...
  methods: {
    ...
    // 获取查询数据
    getSearchData() {
      this.$store.dispatch("fetchSearchData", this.searchParams);
    },
    // 删除属性
    removeAttr(index) {
    ...
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置空，置空后向服务器发起请求
      // 优化 => 接口要求所带参数不是必须的 当属性值为空字符串时 还是会发相应字段
      // 改为undefined 则字段不会发给服务器
